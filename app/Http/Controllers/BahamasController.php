...
class BahamasController extends Controller
{
    protected $api;
    ...
    public function index()
{
    ...
    // default data (always initialise before assigning)
    $data = [
        'total_companies'   => 0,
        'valid_companies'   => 0,
        'invalid_companies' => 0,
    ...
    try {
        ...
        if ($ok) {
            // Note: your response shape used data.data earlier so handle both
    ...
    // -----------------------
    // Delivery SLA — robust parsing (do NOT return json; just populate $data)
    // -----------------------
    try {
    ...
    if (!is_array($slaResp) && !is_object($slaResp)) {
        ...
        if ($ok) {
            // --- TRY multiple likely locations where metrics live ---
            // prefer data.data (your dump shows nested data), fallback to data
            $slaData = data_get($slaResp, 'data.data', data_get($slaResp, 'data', []));

            // now extract metrics from that resolved $slaData
            ...
            // last_updated may also be nested under data.data
    ...
    // Pass to view (Blade) — blade will use $data to populate window.slaDeliveryData and initial chart
    return view('content.dashboard.bahamas-dashboards-analytics', compact('data', 'year'));
}
     ...
     public function fileTypeByYear(Request $request)
    {
        ...
        $summary = data_get($apiData, 'overall_delivery_format_counts', [
            'total_delivered' => 0, 'xbrl' => 0, 'non_xbrl' => 0
        ]);
        return response()->json([
            'success' => true,
            'data' => [
