...
class ProjectController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // If parent is supplied, prefer parent’s customer_id
        if (!empty($parentEncrypted)) {
            try {
                $parentId = Crypt::decryptString($parentEncrypted);
                $parent   = Project::select('id', 'customer_id')->find($parentId);
    ...
    public function edit($encryptedId)
    {
        ...
        $project = Project::with(['pms:id', 'pocs:id'])->findOrFail($id);
        ...
        $customerId = old('customer_id', $project->customer_id);
        $pocsForCustomer = User::when($customerId, fn($q) => $q->where('company_id', $customerId))
    ...
    protected function validateData(Request $request, ?int $projectId = null): array
    {
        ...
        $clean['pm_ids'] = collect($request->input('pm_ids', []))
            ->filter(fn($v) => filled($v))
            ->map(fn($v) => (int) $v)
            ->values()->all();
        ...
        $rules = [
            'project_category'          => ['required', Rule::in([1, 2, 3])],
            'project_name'              => ['required', 'string', 'max:255'],
            'description'               => ['required', 'string', 'max:10000'],
            'is_recurring'              => ['nullable', 'boolean'],
            'recurring_type'            => [
                Rule::requiredIf(fn() => (bool)$request->boolean('is_recurring')),
    ...
    public function store2(Request $request)
    {
        ...
        $formFields = [
            ...
            'property_name','tenant_name','tenant_or_lease_id','premises_address','no_of_documents','pdf_names','sb_queries',
            'type_of_queries','client_response','query_status','query_raised_date','query_resolved_date','abstractor',
            'abstraction_start_date','abstract_completion_date','reviewer','review_start_date','review_completion_date',
            'sense_check_ddr','sense_check_start_date','sense_check_completion_date','proposed_delivery_date',
            'actual_delivered_date','feedback_received_date','feedback_completion_date','billing_month','non_english_pages',
            'invoice_format','fb_date_received','fb_customer_name','fb_category_id','fb_customer_comments','fb_sb_response',
            'fb_feedback_completion_date','cost_usd','type_of_lease','type_of_work','language','suite_id',
        ];
        ...
        $normDate = function ($v) {
            ...
            $fmts = ['Y-m-d','d/m/Y','d-m-Y','m/d/Y','m-d-Y','Y/m/d','d M Y','M d, Y','Y-m-d H:i:s'];
            foreach ($fmts as $f) { try { return \Carbon\Carbon::createFromFormat($f, $s)->format('Y-m-d'); } catch (\Throwable $e) {} }
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $rows[] = [
                ...
                'abstractor_id'                 => data_get($all, "abstractor.{$i}"),
                'abstraction_start_date'        => data_get($all, "abstraction_start_date.{$i}"),
                'abstract_completion_date'      => data_get($all, "abstract_completion_date.{$i}"),
                'reviewer_id'                   => data_get($all, "reviewer.{$i}"),
                'review_completion_date'        => data_get($all, "review_completion_date.{$i}"),
                'sense_check_ddr_id'            => data_get($all, "sense_check_ddr.{$i}"),
                'sense_check_completion_date'   => data_get($all, "sense_check_completion_date.{$i}"),
                'proposed_delivery_date'        => data_get($all, "proposed_delivery_date.{$i}"),
                'actual_delivered_date'         => data_get($all, "actual_delivered_date.{$i}"),
                'feedback_received_date'        => data_get($all, "feedback_received_date.{$i}"),
                'feedback_completion_date'      => data_get($all, "feedback_completion_date.{$i}"),
                'billing_month'                 => data_get($all, "billing_month.{$i}"),
                'non_english_pages'             => (int) (data_get($all, "non_english_pages.{$i}", 0) ?? 0),
                'invoice_format_id'             => data_get($all, "invoice_format.{$i}"),
        ...
        if (empty($rows)) {
            return back()->with('warning', 'Nothing to save.');
        }
        ...
        DB::transaction(function () use ($normalized, $parentId, $canPrune) {
            $idsToKeep = array_values(array_filter(array_map(fn($r) => $r['id'] ?? null, $normalized)));
        ...
        if (!empty($existingIds)) {
            $existingById = \App\Models\ProjectIntake::whereIn('id', $existingIds)->get([
                'id','parent_id','property_name','tenant_name','tenant_or_lease_id',
                'abstract_completion_date','review_completion_date','sense_check_completion_date',
                'abstract_notified_on','review_notified_on','sense_notified_on',
                'reviewer_id','sense_check_ddr_id',
            ])->keyBy('id');
        }
        ...
        DB::transaction(function () use (
            $rows, $parentId, $existingById, $submittedById, $projectName,
            &$notifyReviewer, &$notifySense, &$notifyPmOnSense,
            &$markAbsNotified, &$markRevNotified, &$markSenseNotified,
            $presentColumns, $hasActiveFilters
        ) {
            // Optional prune
            $user = auth()->user();
            $canPrune = $user && ($user->hasRole('super admin') || $user->hasRole('project manager'));
            ...
            DB::afterCommit(function () use (
                $notifyReviewer, $notifySense, $notifyPmOnSense,
                $parentId, $markAbsNotified, $markRevNotified, $markSenseNotified
            ) {
