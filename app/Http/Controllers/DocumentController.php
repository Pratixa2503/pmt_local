        ...
        if (!empty($contractsData)) {
            foreach ($contractsData as $index => $contractData) {
                if (!empty($contractData['contract_start_date']) && !empty($contractData['contract_end_date'])) {
                    ...
                    // Create alerts for this contract if active and alert days are selected
                    if ($contract->is_active && !empty($alertsData[$index]['alert_days'])) {
                        $alertDays = $alertsData[$index]['alert_days'];
                        if (!is_array($alertDays)) {
                            $alertDays = [$alertDays];
                        }
                        
                        // Create one alert record with all selected days
                        ...
                        if (isset($alertFiles['alerts'][$index]['alert_file'])) {
                ...
                if (!empty($contractData['contract_start_date']) && !empty($contractData['contract_end_date'])) {
                    ...
                    if ($contract->is_active && !empty($alertsData[$index]['alert_days'])) {
                        ...
                        if (isset($alertFiles['alerts'][$index]['alert_file'])) {
                            ...
                            if (!empty($alertsData[$index]['existing_file']) && Storage::disk('public')->exists($alertsData[$index]['existing_file'])) {
                            ...
                            $fileName = "alert_{$document->id}_{$originalName}_{$uniqueId}.{$extension}";
                            $alert->alert_file = $file->storeAs('documents/alerts', $fileName, 'public');
                        } elseif (!empty($alertsData[$index]['existing_file'])) {
                            // Keep existing file
