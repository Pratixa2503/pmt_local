...
>>>>>>> 9d9ed85b (for cleaner setup)
        ...
        $project = DB::table('projects')
            ->select('id', 'project_name', 'project_category')
...
<<<<<<< HEAD
                ...
                $projectRate   = isset($pricingRow?->pricing_rate) ? (float) $pricingRow->pricing_rate : null;
                $currencyName  = $pricingRow->currency_name ?? '';                // e.g. "USD", "INR" or full name
...
<<<<<<< HEAD
                    ...
                    ->get()
                    ->keyBy('intake_id');
                // 2) Pull intakes for the month
                $intakes = DB::table('project_intakes')
                    ->select('id', 'property_name', 'tenant_name', 'delivered_date', 'cost_usd', 'billing_month', 'property_id')
                    ->where('parent_id', $project->id)
                    ->where('billing_month', $yyyyMm)
                    ->orderByDesc('id')
                    ->get();

                $rows = $intakes->map(function ($r) use ($projectRate, $locked) {
                    $qty   = 1;
                    $rate  = $projectRate ?? (float)($r->cost_usd ?? 0);
                    $value = $qty * $rate;
...
=======
                ...
                // ---- Intakes for the month ----
                // $intakes = DB::table('project_intakes as pi')
                //     ->select(
                //         'pi.id',
                //         'pi.property_name',
                //         'pi.tenant_name',
                ...
                //     ->orderByDesc('pi.id')
                //     ->get();
                // Resolve "Delivered" status ID once
$deliveredStatusId = DB::table('intake_statuses')
    ->where('name', 'Delivered')
    ->value('id');

// Base builder with your columns
            $base = DB::table('project_intakes as pi')
                ...
                ->where('pi.parent_id', $project->id)     // or $project->id if you prefer
                ->where('pi.billing_month', $yyyyMm)
            ...
            if ($deliveredStatusId) {
                ...
                // Fallback (in case status row doesn't exist in this env)
            ...
            $intakes = $base->orderByDesc('pi.id')->get();
                ...
                $resolveApprovedPricing = function ($serviceOfferingId) use ($ivId, $depId, $project) {
                    ...
                    // 1) Try customer-specific STANDARD pricing first
                    $row = DB::table('pricing_masters as pm')
                        ...
                        ->where(function ($q) use ($project) {
                            // customer-specific row
                            $q->where('pm.customer_id', $project->id); // if you actually store company id here, change to $project->customer_id
                        })
                        ->select('pm.id', 'pm.rate', 'pm.approval_status', 'cur.name as currency_name')
                        ->orderByDesc('pm.id')
                        ->first();

                    // 2) If not found, fallback to global STANDARD pricing (no customer)
                    if (!$row) {
                        $row = DB::table('pricing_masters as pm')
                            ->leftJoin('currencies as cur', 'cur.id', '=', 'pm.currency_id')
                            ->whereNull('pm.deleted_at')
...
>>>>>>> 9d9ed85b (for cleaner setup)
            ...
            case 3:
                // ---------------- Other / Category 3: placeholder branch ----------------
                // TODO: plug your cat=3 invoice logic here
                return response()->json([
                    'category' => 3,
                    'source'   => 'custom_logic_cat3', // change when you implement
                    'project'  => ['id' => $project->id, 'name' => $project->project_name],
                    'data'     => [],
                    'message'  => 'No invoice logic implemented for Category 3 yet.',
                ]);
...
<<<<<<< HEAD
        ...
        //     ->select('name', 'address', 'customer_type', 'country')
        //     ->first();

        // // Decide template by customer_type (fallback by country)
        // $customerType = strtoupper(trim($customer->customer_type ?? ''));
        // if (!$customerType) {
        //     $country = strtoupper((string)($customer->country ?? ''));
        //     $customerType = ($country === 'INDIA' || $country === 'IN') ? 'IND' : 'US';
        // }

        // ---- Currency symbol (config/currency.php → symbols map) ----
        $currencyKey    = strtoupper(trim($project->currency_name ?? ''));
        $currencySymbol = config('currency.symbols.' . $currencyKey, '');
        $currencyName   = $project->currency_name ?? '';

        // ---- Intake rows ----
        $rows = DB::table('project_intakes')
            ->select('id', 'property_name', 'tenant_name', 'cost_usd', 'property_id', 'billing_month')
            ->where('parent_id', $request->project_id)
            ->when($request->filled('month'), fn($q) => $q->where('billing_month', $request->month))
            ->whereIn('id', $request->intake_ids)
            ->orderBy('id')
            ->get();

        $projectRate = isset($project->pricing_rate) ? (float)$project->pricing_rate : null;

        $lines = $rows->values()->map(function ($r, $idx) use ($projectRate, $sacCodeDefault) {
            $unitRate = $projectRate ?? (float)($r->cost_usd ?? 0);
            $qty      = 1;
            $value    = $qty * $unitRate;

            return (object)[
                'sno'         => $idx + 1,
                'description' => trim(($r->property_name ?? '-') . ' (' . ($r->tenant_name ?? '-') . ') — ID: ' . ($r->property_id ?? '-')),
                'sac'         => $sacCodeDefault, // visible in IND template, harmless in US
                'qty'         => $qty,
                'rate'        => $unitRate,
                'value'       => $value,
            ];
...
=======
        ...
        $resolveStandardPricing = function ($ivId, $deptId, $serviceOfferingId) use (&$pricingCache) {
            ...
            $row = DB::table('pricing_masters as pm')
                ...
                ->where('pm.service_offering_id', $serviceOfferingId)
                ->orderByDesc('pm.id')
                ->select(['pm.rate', 'c.name as currency_name'])
                ->first();
        ...
        // ---------------- Group rows BY service offering ----------------
        // For each row: compute its effective unit rate (standard->project->cost_usd), value, then sum per SO.
        $grouped = []; // service_offering_id => ['count'=>n, 'sum_value'=>x, 'std_rate'=>float|null]
        foreach ($rows as $r) {
        ...
        foreach ($grouped as $soId => $agg) {
            ...
            // Prefer a clear unit rate to display:
            //   1) standard approved rate for this SO
            //   2) project rate (if exists and used globally)
            //   3) fallback to average (sum / qty)
...
<<<<<<< HEAD
        ...
        $subProjectId = $projMeta?->parent_id ? (int)$request->project_id : null;

        $poNo = '';
        if ($request->filled('month')) {
            try {
                $dtStart = Carbon::createFromFormat('Y-m', $request->month)->startOfMonth();
                $dtEnd   = $dtStart->copy()->endOfMonth();

                $poQ = DB::table('po_numbers')
                    ->whereNull('deleted_at')
                    ->where('status', 1)
                    ->where('project_id', $parentId)
                    ->where(function ($q) use ($dtStart, $dtEnd) {
                        $q->where('start_date', '<=', $dtEnd)
                            ->where('end_date', '>=', $dtStart);
                    });

                is_null($subProjectId)
                    ? $poQ->whereNull('sub_project_id')
                    : $poQ->where('sub_project_id', $subProjectId);

                $po = $poQ->orderByDesc('id')->first();
                if ($po) $poNo = $po->po_number ?? '';
...
=======
        ...
        $poNo = '';
        try {
            $dtStart = \Carbon\Carbon::createFromFormat('m-Y', $monthDb)->startOfMonth();
            $dtEnd   = $dtStart->copy()->endOfMonth();
...
=======
    ...
    public function approve(Request $request)
    {
        ...
        // ---------------- 1) Duplicate check using invoice_lines ----------------
        ...
        $projectRate     = isset($project->pricing_rate) ? (float) $project->pricing_rate : null;
        $currencyId      = $project->currency_id ?? null;
        $currencyName    = $project->currency_name ?? '';
        $currencyKey     = strtoupper(trim($currencyName));
        $currencySymbol  = config('currency.symbols.' . $currencyKey, '');
        ...
        // ---------------- 3) Fetch the selected intakes (with month + SO) ----------------
        $intakes = DB::table('project_intakes')
            ->whereIn('id', $intakeIds)
            ->where('billing_month', $yyyyMm)               // ensure same month
            ->get(['id', 'type_of_work_id', 'cost_usd']);   // no need for property/tenant now
        ...
        $projMeta     = DB::table('projects')->where('id', $projectId)->select('id', 'parent_id')->first();
        $parentId     = $projMeta?->parent_id ? (int)$projMeta->parent_id : (int)$projectId;
        $subProjectId = $projMeta?->parent_id ? (int)$projectId : null;

        $poNo = '';
        try {
            $dtStart = \Carbon\Carbon::createFromFormat('m-Y', $yyyyMm)->startOfMonth();
            $dtEnd   = $dtStart->copy()->endOfMonth();
        ...
        // If project currency is missing, adopt the first standard pricing currency we find
        if (!$currencyId || !$currencyName) {
            foreach ($intakes as $r) {
                ...
                if ($pm && $pm->currency_id && $pm->currency_name) {
                    $currencyId     = $pm->currency_id;
                    $currencyName   = $pm->currency_name;
                    $currencyKey    = strtoupper(trim($currencyName));
                    $currencySymbol = config('currency.symbols.' . $currencyKey, '');
                    break;
        ...
        // ---------------- 5) Create Invoice + Lines (per intake; description = SO name) ----------------
        $invoiceId = DB::transaction(function () use (
            ...
            // ---- Lines (PER INTAKE; description = Service Offering name; pricing by priority) ----
            $sno = 1;
            foreach ($intakes as $r) {
                $soId   = (int)($r->type_of_work_id ?? 0);
        ...
        $financeTo = "keyur.padaria@briskstar.com";
        //$financeCc = ['accounts@yopmail.com'];
