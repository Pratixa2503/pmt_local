...
class TaskTrackerController extends Controller
{
    public function generalView(string $encryptedId, Request $request)
    {
        ...
        $userId    = (int) auth()->id();
        $productiveMainTasks = MainTask::where('task_type', 1)->orderBy('name')->pluck('name', 'id');
    ...
    public function start(Request $request)
    {
        ...
        return DB::transaction(function () use ($user, $data, $nowLocal, $tz) {
            ...
            $targetItem = TaskItem::where([
                ...
                if (!empty($newNotes)) {
                    // Trim whitespace from new notes to clean up presentation
                    $cleanedNewNotes = trim($newNotes); 
                    
                    // Ensure the separator starts on a new line and use \n consistently
                    $separator = "\n---\n" . $nowLocal->format('Y-m-d H:i:s') . " Notes:\n";
                    
                    // Add a final trim to the whole notes field to avoid extra blank lines at the start/end
                    $item->notes = trim($item->notes . $separator . $cleanedNewNotes);
                }
   ...
   public function pause(Request $request)
{
    ...
    $tz = config('app.timezone', 'Asia/Kolkata');
    $user = Auth::user();
    $taskItemId = $validated['task_item_id'];
    $newCount = (int)$validated['counts'];
    $newNotes = $validated['notes'];
    $nowLocal = now($tz);

    return DB::transaction(function () use ($user, $taskItemId, $newCount, $newNotes, $nowLocal) {
        
        // 2. Find the current open WorkSession
        ...
        // 3. Update the WorkSession record with the collected data and end time.
        // NOTE: This assumes you have 'count' and 'notes' columns in your 'work_sessions' table.
        $openSession->ended_at = $nowLocal;
        $openSession->counts    = $newCount; // Save count to the session
        $openSession->notes    = $newNotes; // Save notes to the session
        $openSession->save();


        // 4. Update TaskItem's total_seconds and total_counts

        $item = TaskItem::lockForUpdate()->findOrFail($taskItemId);

        // a. Calculate duration for total_seconds
        $durationSeconds = $openSession->started_at->diffInSeconds($nowLocal);
        $item->total_seconds += max($durationSeconds, 1); // Ensure at least 1 second is added

        // b. Accumulate the new count into total_counts
        $item->total_counts += $newCount; 

        // c. Append notes to TaskItem's notes (optional, if you want history in the main table)
        if (!empty($newNotes)) {
            $separator = "\n---\n" . $nowLocal->format('Y-m-d H:i:s') . " Notes:\n";
            $item->notes = trim($item->notes . $separator . $newNotes);
        }

        // Set status to paused (2)
        $item->status = 2;
        $item->save();
        
        // --- NEW/MODIFIED LOGIC ENDS HERE ---
        
        // 5. Return success response
        return response()->json([
            'status'        => 1,
            'message'       => 'Task paused and data saved.',
            'task_item_id'  => $item->id,
            'total_seconds' => (int)$item->total_seconds,
            'total_counts'  => (int)$item->total_counts, // Return new total counts
        ]);
    });
}
/*
    ...
    public function todayFragment(string $encryptedId)
    {
        ...
        // "today" window in app TZ (strict local)
        ...
        $todayRows = DB::table('work_sessions as ws')
            ...
            ->where(function ($q) use ($startUtc, $endUtc) {
                // session intersects the [startLocal, endLocal] window
                $q->where('ws.started_at', '<=', $endUtc)
                    ->where(function ($q2) use ($startUtc) {
                        $q2->whereNull('ws.ended_at')
                            ->orWhere('ws.ended_at', '>=', $startUtc);
                    });
            })
            ->select([
                'ti.id as task_item_id',
            ...
            ->groupBy('ti.id', 'mt.name', 'st.name')
            ->orderByDesc('ti.id')
            ->setBindings([$startUtc, $nowLocal, $endUtc], 'select')
            ->get();
        $secondsToday = (int) $todayRows->sum(fn($r) => (int) $r->seconds_today);
    ...
    public function adminExport(string $encryptedId, Request $request)
    {
        ...
        $rows = DB::table('work_sessions as ws')
            ...
            ->select([
                ...
                'mt.name as main_task',
                'st.name as sub_task',
                'ti.total_counts', // ✅ Add total_counts from task_items
                'ti.notes',
                DB::raw("
                    SUM(
                    GREATEST(
    ...
    /* public function getSessionHistory(Request $request)
    {
        ...
        $sessions = WorkSession::where('task_item_id', $request->input('task_item_id'))
                            ->whereNotNull('ended_at')
                            ->orderBy('ended_at', 'desc')
                            ->get(['ended_at', 'counts', 'notes']); // Select only necessary columns
        //dd($sessions);

        return response()->json(['status' => 1, 'sessions' => $sessions]);
    } */
...
public function getSessionHistory(Request $request)
{
    ...
    $request->validate(['id' => 'required|integer|exists:task_items,id']);
    
    $taskItemId = $request->input('id');
