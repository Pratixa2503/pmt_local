...
<html>
<head>
    <title>Document Contract Start Date Alert</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h2>Document Contract Start Date Alert</h2>
    </div>

    <p>Hello <strong>{{ $customer->name ?? 'Customer' }}</strong>,</p>

    <div class="alert-box">
        <strong>⚠️ Alert:</strong> Your document contract is scheduled to start in <strong>{{ $daysUntilStart }} {{ $daysUntilStart == 1 ? 'day' : 'days' }}</strong>.
    </div>
    ...
    <div class="info-box">
        ...
        <div class="details">
            <table>
                ...
                @endif
                @if($alert && $alert->alert_days)
                <tr>
                    <td>Alert Before:</td>
                    <td>{{ $alert->alert_days }} Days</td>
                </tr>
                @endif
            </table>
        </div>
