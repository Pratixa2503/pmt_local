...
<head>
<meta charset="utf-8">
<title>{{ $brand }} — Invoice {{ $statusText }}</title>
          ...
          <tr>
            <td class="card" style="padding:28px 28px 8px 28px;">
              <h1 style="margin:0 0 12px 0; font-size:20px; font-weight:700; color:#111827;">
                Invoice {{ $statusText }}: {{ $invoiceNo }}
              </h1>
              <p style="margin:0 0 18px 0; font-size:14px; color:#555;">
                Your invoice has been {{ $isApproved ? 'approved' : 'rejected' }} by the finance team.
              </p>

              <div style="background:{{ $statusColor }}15; border-left:4px solid {{ $statusColor }}; padding:12px 16px; margin:0 0 16px 0; border-radius:6px;">
                ...
                <tr>
                  <td style="background:#FFFFFF; border:1px solid #97999B66; border-radius:10px; padding:14px;">
                    <table width="100%">
                      ...
                      @if($financeNotes)
                      <tr>
                        <td colspan="2" style="font-size:14px; color:#111827; padding:12px 0 6px 0; border-top:1px solid #97999B33; margin-top:8px;">
                          <strong>Finance Notes:</strong>
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2" style="font-size:13px; color:#555; padding:6px 0;">
                          {{ $financeNotes }}
                        </td>
                      </tr>
                      @endif
