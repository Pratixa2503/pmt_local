...
@section('title', 'Input/Output Formats')
...
<div class="card">
  <div class="card-header">
    <div class="d-flex justify-content-between">
      <h4 class="text-dark bold mb-0">Input/Output Formats</h4>
      @can('create input output format')
        <a href="{{ route('input-output-formats.create') }}" class="btn btn-primary btn-md" title="Add Format">
          <i class="fa fa-plus me-2"></i>Add Format
        </a>
      @endcan
    </div>
  </div>
...
<script>
  ...
  $(function () {
    $('#input-output-format-table').DataTable({
      ...
      processing: true,
      serverSide: true,
      ajax: '{!! route('input-output-formats.index') !!}',
  ...
  $(document).on("click", "#delete-input-output-format", function () {
    var id = $(this).data('id');
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: "{{ url('input-output-formats') }}/" + id,
          method: "DELETE",
          data: {
            '_token': '{{ csrf_token() }}',
          },
