{{-- resources/views/content/invoice_formats/index.blade.php --}}
@php
...
<div class="card">
  <div class="card-header">
    <div class="justify-content-between d-flex">
      <h4 class="text-dark bold mb-0">{{ __('Invoice Formats') }}</h4>
      <div class="justify-content-between">
        @if(auth()->check() && auth()->user()->can('create invoice format'))
          <a href="{{ route('invoice-formats.create') }}"
             class="btn btn-primary btn-md"
             data-toggle="tooltip"
             title="Add Invoice Format">
            <i class="fa fa-plus me-2"></i>{{ __('Add Invoice Format') }}
          </a>
        @endif
      </div>
    </div>
  ...
  <div class="card-body">
    <div class="table-responsive text-nowrap">
      ...
      <table class="common-datatable table" id="invoiceformat-table">
        <thead>
          <tr>
            <th></th>
            <th>Name</th>
...
<script>
  ...
  $(function () {
    const $tbl = $('#invoiceformat-table');

    $tbl.DataTable({
      responsive: true,
      processing: true,
      serverSide: true,
      ajax: '{!! route('invoice-formats.index') !!}',
    ...
    // Delete (expects actions column to have .delete-invoice-format with data-id = encrypted id)
    $(document).on('click', '.delete-invoice-format', function () {
      const encId = $(this).data('id');

      Swal.fire({
        text: 'Are you sure you want to delete this Invoice Format?',
        icon: 'warning',
        ...
        if (result.value) {
          $.ajax({
            url: "{{ route('invoice-formats.destroy') }}", // POST route that hits destroy(Request $request)
            method: "DELETE",
            data: {
              _token: '{{ csrf_token() }}',
              id: encId
