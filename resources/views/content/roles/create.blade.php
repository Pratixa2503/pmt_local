...
@section('content')
    {{--  <x-breadcrumb :items="['home' => 'Roles and Permissions', 'icon' => 'menu-icon tf-icons ti ti-user-check', 'name' => 'Roles', 'subname' => 'Add Role']" /> --}}
        ...
        <div class="col-xxl">
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 text-dark bold">{{ __('Add Role') }}</h4>
                </div>
                <div class="card-body">
                    <form id="createForm" method="POST" action="{{ route('roles.store') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="mb-3 custom-validation col-md-6 col-lg-6">
                                <label for="name" class="form-label">Role Name</label>
                                <input type="text" placeholder="Enter Role Name" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" name="name" value="{{ old('name') }}">
                                @error('name')
            ...
            $("#createForm").validate({
                ...
                messages: {
                    name: {
                        ...
                        regex: "Role Name can contain only small letters and spaces",
                    },
                    'permissions[]': {
