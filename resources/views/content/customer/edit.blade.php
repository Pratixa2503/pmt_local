        ...
        <div class="col-xxl">
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 text-dark bold">{{ __('Edit Customer') }}</h4>
                </div>
                <div class="card-body">
                    <form id="createForm" method="POST" action="{{ route('customers.update', Crypt::encryptString($customer->id)) }}" enctype="multipart/form-data">
                        @csrf 
                        @method('PUT')
                        <div class="row">
                            @php
                            ...
                            <div class="mb-3 custom-validation col-md-6 col-lg-6">
                                ...
                                <select name="role" id="role" class="form-control form-select {{ $errors->has('role') ? 'is-invalid' : '' }}">
                                    ...
                                    @foreach ($roles as $role)
                                        <option value="{{ old('role', $role->name) }}" @if($customer->role_id == $role->id) selected @endif>{{ ucwords($role->name) }}</option>
                                    @endforeach
                                </select>
                                @error('role')
                                    <span class="invalid-feedback d-block" role="alert">
                            ...
                            <div class="mb-3 custom-validation col-md-6 col-lg-6">
                                ...
                                <select name="status" id="status" class="form-control form-select {{ $errors->has('status') ? 'is-invalid' : '' }}">
                                    ...
                                    <option value="0" {{ old('status', $customer->status) == '0' ? 'selected' : '' }}>Deactive</option>
                                </select>
                                @error('status')
                                    <span class="invalid-feedback d-block" role="alert">
                                        <strong>{{ $message }}</strong>
