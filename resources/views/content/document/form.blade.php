        ...
        <form id="documentForm" method="POST" action="{{ $action }}" enctype="multipart/form-data" novalidate>
          ...
          <div class="row g-3">
            <div class="col-md-4">
              ...
              <select name="customer_id" id="customer_id" class="form-select select2" required>
                ...
                <option value="{{ $c->id }}" {{ (string)old('customer_id', $document->customer_id ?? '') === (string)$c->id ? 'selected' : '' }}>
                  {{ $c->name }}
                </option>
                @endforeach
              </select>
            ...
            <div class="col-md-4">
             ...
             <input type="text" name="contact_no" id="contact_no" class="form-control"
              value="{{ old('contact_no', isset($document->contact_no) ? $document->contact_no : '') }}">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input type="text" class="form-control datepicker" id="contract_start_date" name="contract_start_date" required
                value="{{ old('contract_start_date', isset($document->contract_start_date) ? \Carbon\Carbon::parse($document->contract_start_date)->format('m/d/Y') : '') }}"
                placeholder="MM/DD/YYYY" autocomplete="off">
              @error('contract_start_date') <div class="text-danger small">{{ $message }}</div> @enderror
            </div>
            <div class="col-md-6">
              <label for="contract_end_date" class="form-label">Contract End Date <span class="text-danger">*</span></label>
              <input type="text" class="form-control datepicker" id="contract_end_date" name="contract_end_date" required
                      ...
                      <div class="col-md-12">
                        ...
                        <div class="form-check mb-2">
                          ...
                          <label class="form-check-label fw-semibold" for="alert_before_master_{{ $index }}">
                            Alert Before (Days) <span class="text-danger">*</span>
                          </label>
                        </div>
          ...
          <div class="row g-3">
            <div class="mb-3 custom-validation col-md-4 col-lg-4">
                ...
                <select name="status" id="status" class="form-control form-select {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <option value="1" {{ old('status', $document->status ?? '') == '1' ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ old('status', $document->status ?? '') == '0' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                ...
                <div class="mb-3">
                    ...
                    <div class="form-text">
                        <small class="text-muted">
                            <i class="ti ti-info-circle"></i> Allowed file types: PDF, DOC, DOCX, XLS, XLSX, PNG, JPG, JPEG. Maximum file size: 5MB (5120 KB)
                        </small>
                    </div>
                    @error('file_path')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
...
<script>
  $(function() {
    ...
    $('#contract_start_date').datepicker({
      ...
      const endD = $('#contract_end_date').datepicker('getDate');
      if (endD && endD < e.date) $('#contract_end_date').datepicker('setDate', null);
    ...
    (function clampOnLoad() {
      const s = parseUsDate($('#contract_start_date').val());
      if (s) {
        $('#contract_end_date').datepicker('setStartDate', s);
        const e = parseUsDate($('#contract_end_date').val());
  ...
  $(document).ready(function() {
    $('#department_id').on('change', function() {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          $pricingSel.trigger('change'); // refresh select2 if applied
        },
        error: function() {
    ...
    // Auto-load once if department already selected (edit case)
