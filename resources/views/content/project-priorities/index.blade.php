...
@section('title', 'Project Priorities')
...
<div class="card">
    ...
    <div class="justify-content-between d-flex">
        <h4 class="text-dark bold mb-0">{{ __('Project Priorities') }}</h4>
        <div class="justify-content-between">
            @if( auth()->check() && auth()->user()->can('create project priorities') )
                <a href="{{ route('project-priorities.create') }}" class="btn btn-primary btn-md" data-toggle="tooltip" title="Add Project Priorities"><i class="fa fa-plus me-2"></i>{{ __('Add Project Priority') }}</a>
            @endif
        </div>
    </div>
    </div>
    ...
    <script>
        ...
        $(function() {
            $('#projecttype-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{!! route('project-priorities.index') !!}',
        ...
        $( document ).on("click", "#delete-project-priorities", function() {
            var id = $(this).data('id');
            Swal.fire({
