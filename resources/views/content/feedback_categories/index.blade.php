...
@section('title', $title ?? 'Feedback Categories')
...
<div class="card">
  <div class="card-header">
    <div class="d-flex justify-content-between">
      <h4 class="text-dark mb-0">{{ $title ?? 'Feedback Categories' }}</h4>
      <div>
        @if(auth()->check() && auth()->user()->can('create feedback category'))
          <a href="{{ route('feedback-categories.create') }}" class="btn btn-primary btn-md">
  ...
  <div class="card-body">
    <div class="table-responsive text-nowrap">
      <table class="common-datatable table" id="feedback-categories-table">
        <thead>
          <tr>
            <th></th>
            <th>Name</th>
...
<script>
  ...
  $(function () {
    const table = $('#feedback-categories-table').DataTable({
      responsive: true,
      processing: true,
      serverSide: true,
      ajax: '{!! route('feedback-categories.index') !!}',
    ...
    // Delete handler
    $(document).on('click', '.delete-feedback-category', function () {
      const id = $(this).data('id');
      Swal.fire({
        text: 'Are you sure you want to delete this Feedback Category?',
        icon: 'warning',
        ...
        if (result.value) {
          $.ajax({
            url: "{{ route('feedback-categories.destroy') }}",
            method: "DELETE",
            ...
            success: function (res) {
              if (res.status) {
                ...
                Swal.fire({ icon: 'error', text: res.message || 'Failed to delete.' });
              }
            },
            error: function () {
