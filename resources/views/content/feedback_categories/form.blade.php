      ...
      <div class="row">
        ...
        <div class="mb-3 col-md-6">
          ...
          <select name="status" class="form-select @error('status') is-invalid @enderror">
            @php $current = old('status', isset($model) ? (int)$model->status : 1); @endphp
            <option value="1" {{ (string)$current === '1' ? 'selected' : '' }}>Active</option>
            <option value="0" {{ (string)$current === '0' ? 'selected' : '' }}>Inactive</option>
          </select>
