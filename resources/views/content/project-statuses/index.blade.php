...
<div class="card">
    ...
    <div class="justify-content-between d-flex">
        <h4 class="text-dark bold mb-0">{{ __('Project Statuses') }}</h4>
        <div class="justify-content-between">
            @if( auth()->check() && auth()->user()->can('create project status') )
                <a href="{{ route('project-statuses.create') }}" class="btn btn-primary btn-md" data-toggle="tooltip" title="Add Project Priorities"><i class="fa fa-plus me-2"></i>{{ __('Add Project Status') }}</a>
            @endif
        </div>
    </div>
    </div>
    ...
    <div class="card-body">
        <div class="table-responsive text-nowrap">
            ...
            <table class="common-datatable table" id="project-statues-table">
                <thead>
                    <tr>
                        <th></th>
                        <th>Name</th>
...
@section('extra-script')
    <script>
        ...
        $(function() {
            $('#project-statues-table').DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                ajax: '{!! route('project-statuses.index') !!}',
        ...
        $( document ).on("click", "#delete-project-status", function() {
            var id = $(this).data('id');
            Swal.fire({
                text: 'Are you sure you want to delete this Project Status?',
                icon: 'warning',
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.status === true) {
                                ...
                                var table = $('#project-statues-table').DataTable();
                                table.ajax.reload();
                            } else {
                                Swal.fire({
                                    icon: 'error',
