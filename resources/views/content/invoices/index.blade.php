...
<script>
  ...
  $(function() {
      ...
      $.post('{{route('invoices.previewData')}}', {
        ...
        .done(function(res) {
          ...
          res.data.forEach(function(r) {
            tbody.append(
              '<tr>' +
              // checkbox must be intake_ids[] so previewHtml receives it
              '<td><input type="checkbox" class="row-check" name="intake_ids[]" value="' + r.id + '"></td>' +
              '<td>' + nn(r.id, '-') + '</td>' +
              '<td><div class="fw-medium">' + nn(r.property_name, '-') + '</div>' +
              '<div class="text-muted small">ID: ' + nn(r.property_id, '-') + '</div></td>' +
              '<td>' + nn(r.tenant_name, '-') + '</td>' +
              '<td>' + nn(r.delivered_date, '-') + '</td>' +
         ...
         res.data.forEach(function(r) {
            ...
            tbody.append(
              '<tr class="'+ (isLocked ? 'row-locked' : '') +'">' +
                '<td><input type="checkbox" class="row-check" name="intake_ids[]" value="' + r.id + '" ' + (isLocked ? 'disabled' : '') + '></td>' +
                '<td>' + nn(r.id, '-') + '</td>' +
    ...
    function rebindSelection() {
      ...
      function update() {
        const sel = $checks.filter(':checked').length;
        $count.text(sel);
        $btnPreview.prop('disabled', sel === 0);
        $selectAll.prop('checked', sel && sel === $checks.length);
        $selectAll.prop('indeterminate', sel > 0 && sel < $checks.length);
      }
      ...
      $table.off('change', '.row-check').on('change', '.row-check', update);
...
@endsection
  ...
  $('#btnApprove').on('click', function () {
    ...
    $.post('{{ route('invoices.approve') }}', {
      project_id: pid,
      month: month,
      'intake_ids': ids   // array payload
      // , bank_id: $('#bankSelect').val() // if/when you add bank selection
    })
    .done(function (res) {
      if (res && res.status === 1) {
        // Close modal
