{{-- India invoice layout (GST/PAN/IFSC etc.) --}}
<style>
...
<div class="grid-2 mt-4">
  <div>
    <table class="tbl">
      ...
      <tr><th>LUT No:</th><td>{{ $lutNo ?: '—' }}</td></tr>
...
@php
  ...
  // If you apply GST (e.g., 18%) for IND:
  $gstRate = 18;
  $gstAmt  = round(($net * $gstRate)/100, 2);
  $netIncl = $net + $gstAmt;
@endphp
...
<div class="sum mt-4">
  <table class="tbl">
    ...
    <tr>
      <th>GST {{ $gstRate }}%</th>
      <td class="text-end">{{ ($currency['symbol'] ?? '') }}{{ number_format($gstAmt,2) }}</td>
    </tr>
    <tr>
      <th>Net Invoice Value</th>
      <td class="text-end fw-bold">{{ ($currency['symbol'] ?? '') }}{{ number_format($netIncl,2) }}</td>
    </tr>
  </table>
</div>
...
<div class="mt-3">
  ...
  <div class="muted">{{ $invoiceDescription ?: 'Any comments from your Finance team' }}</div>
</div>
