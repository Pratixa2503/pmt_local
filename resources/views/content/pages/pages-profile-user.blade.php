...
@section('content')
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">{{ __('My Profile') }} /</span> {{ __('Profile') }}
    </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <ul class="nav nav-pills flex-column flex-sm-row mb-4">
               <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><i class='ti-xs ti ti-user-check me-1'></i> {{ __('Profile') }}</a></li>
                <li class="nav-item"><a class="nav-link" href="{{ route('update-password') }}"><i class='ti-xs ti ti-layout-grid me-1'></i> {{ __('Change Password') }}</a></li>
            </ul>
        </div>
    </div>
    <!--/ Navbar pills -->
    ...
    <div class="row">
        <div class="col-xl-6 col-lg-6 col-md-6">
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between accordian-item">
                        <small class="card-text bold text-uppercase mr-0">About</small>
                        <a data-id="{{ Auth::user()->id }}" class="link px-2 profile-edit mt-2"  data-toggle="tooltip" title="Edit"><i class="ti ti-edit ti-sm"></i></a>
                    </div>
                    <form id="editNameForm" class="full-name-edit d-none">
                        <span class="fw-bold"> <i class="ti ti-user mb-1"></i> Full Name:</span>
                        <div class="full-name-edit d-none d-flex mt-2 mb-3">
                            <div class="form-floating custom-validation">
                                <input type="text" class="form-control d-inline-block first_name " name="first_name" value="{{ Auth::user()->first_name }}" placeholder="First Name" />
                                <label for="name">First Name</label>
                            </div>
                            <div class="form-floating mx-2 custom-validation">
                                <input type="text" class="form-control d-inline-block last_name" name="last_name" value="{{ Auth::user()->last_name }}" placeholder="Last Name" />
                                <label for="lastName">Last Name</label>
                            </div>
                            <input type="hidden" name="userId" value="{{ Auth::user()->id }}" />
                        </div>
                        <span class="fw-bold"> <i class="ti ti-user mb-2"></i> Contact:</span>
                        <div class="contact-edit d-none d-flex mt-2">
                            <div class="form-floating custom-validation">
                                <input type="text" class="form-control d-inline-block contact_no price_val" name="contact_no" value="{{ Auth::user()->contact_no }}" placeholder="Contact No" />
                                <label for="contact_no">Contact No</label>
                            </div>
                            <div class="form-floating mx-2 custom-validation">
                                <input type="text" class="form-control d-inline-block company_name" name="company_name" value="{{ Auth::user()->company_name }}" placeholder="Contact No" />
                                <label for="company_name">Company Name</label>
                            </div>
                            <div class="text-end" style="margin-top: 12px;">
                                <a data-id="{{ Auth::user()->id }}" class="px-0 accordian-save d-none"
                                    data-toggle="tooltip" title="Save"><i class="ti ti-device-floppy text-primary ti-lg "></i>
                                </a>
                            </div>
                        </div>
                    </form>
                    <ul class="list-unstyled mb-4 mt-3">
                        <li class="d-flex align-items-center mb-3 edit-class">
                            <i class="ti ti-user edit-class"></i>
                            <span class="fw-bold mx-2 edit-class">Full Name:</span>
                        ...
                        <li class="d-flex align-items-center mb-3"><i class="ti ti-check"></i><span
                            ...
                            <span>{{ Auth::user()->status == '1' ? 'Active' : 'Deactive' }}</span>
                        </li>
                        ...
                        <li class="d-flex align-items-center mb-3 edit-class"><i class="ti ti-crown edit-class"></i><span
                                class="fw-bold mx-2 edit-class">Company Name:</span> <span class="company-name-display">
                                {{ Auth::user()->company_name }}
                            </span>
                        </li>
                    </ul>
                    <small class="card-text bold text-uppercase">Contacts</small>
                    <ul class="list-unstyled mb-4 mt-3">
                        <li class="d-flex align-items-center mb-3 edit-class"><i class="ti ti-phone-call edit-class"></i><span class="fw-bold mx-2 edit-class">Contact:</span> <span class="contact-no-display">{{ Auth::user()->contact_no }}</span></li>
                        <li class="d-flex align-items-center mb-3"><i class="ti ti-mail"></i><span class="fw-bold mx-2">Email:</span> <span>{{ Auth::user()->email }}</span></li>
                    </ul>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.price_val').on('keypress', function(evt) {
            var currentValue = $(this).val();
            var dotIndex = currentValue.indexOf('.');
            if (evt.which === 46 && dotIndex === -1) {
                return;
            }
            if (evt.which != 8 && evt.which != 46 && (evt.which < 48 || evt.which > 57)) {
                evt.preventDefault();
                return;
            }
            if (dotIndex !== -1 && evt.which === 46) {
                evt.preventDefault();
                return;
            }
        });
        ...
        $(document).on('click', '.profile-edit', function() {
            // Toggle visibility of the display and edit elements
            $('.full-name-display').toggleClass('d-none');
            $('.full-name-edit').toggleClass('d-none');
            $('.edit-class').toggleClass('d-none');
            $('.accordian-save').toggleClass('d-none');
            $('.profile-edit').toggleClass('d-none');
            $('.ti-edit').toggleClass('d-none');
            $('.contact-edit').toggleClass('d-none');
            $('.contact-display').toggleClass('d-none');
            $('.contact-no-display').toggleClass('d-none');
            $('.company-name-display').toggleClass('d-none');
        ...
        $(document).on('keyup', '.contact_no', function() {
            $(this).valid();
        });
        ...
        $(document).on('click', '.accordian-save', function(event) {
        ...
        function submitForm(formSelector) {
            ...
            var form = $('.accordian-save').closest('.card-body').find(formSelector);
            ...
            $.ajax({
                url: "{{ url('/pages/profile-user-update') }}",
                type: 'POST',
                data: serialized,
                success: function(response) {
                    if (response.status) {
                        $('.full-name-display').text(response.first_name + ' ' + response.last_name);
                        $('.company-name-display').text(response.company_name);
                        $('.contact-no-display').text(response.contact_no);
                        $('.full-name-display').toggleClass('d-none');
                        $('.company-name-display').toggleClass('d-none');
                        $('.contact-no-display').toggleClass('d-none');
                        $('.full-name-edit').toggleClass('d-none');
